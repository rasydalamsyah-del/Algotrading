...
class CoinProfile:
    ...
    def max_hold_seconds(self) -> int:
        if self.max_hold_candles <= 0:
            return 0
        # [DEDUP] Sebelumnya dict tf->detik terduplikasi lokal di sini,
        # terpisah dari engine.constants.TIMEFRAME_SECONDS yang isinya
        # PERSIS SAMA (14 entri identik) -- risiko drift kalau salah satu
        # diupdate tapi yang lain lupa. Sekarang reference satu sumber yang
        # sama, juga dipakai future/capital_allocator.py.
        secs = TIMEFRAME_SECONDS.get(self.timeframe, 900)
